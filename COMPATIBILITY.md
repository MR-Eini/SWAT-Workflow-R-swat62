...
The legacy 53-column plant table was also tested directly against revision 62. It caused every even-numbered plant row to be skipped and produced `agrc` substitutions. The migrated 56-column table preserved all 268 supplied plants, completed the 2004–2023 run, produced no unresolved-plant diagnostics, and reported the intended crop names. Details are in [PLANTS-SWAT62.md](PLANTS-SWAT62.md).

This evidence covers the supplied model and its configured processes. The migration helper deliberately refuses carbon-enabled inputs; the added lignin fractions are the SWAT+ internal defaults for non-carbon runs. The Windows GNU revision 62 build stopped during weather initialization for this model; the Intel revision 62 build is the tested executable. Successful execution shows software compatibility, while calibration quality and scientific acceptance require a separate model assessment.
