...
swat62_library <- Sys.getenv('SWAT_PACKAGE_LIBRARY', file.path(swat_workflow_root(), 'package-updates/library'))
...
swat62_versions <- c(SWATreadR = '0.1.0.9013', SWATrunR = '1.1.0.9019',
  ...
  SWATprepR = '1.0.14', SWATmeasR = '0.9.4')
swat62_require <- function(packages = names(swat62_versions)) {
  for (pkg in packages) {
    if (!requireNamespace(pkg, quietly = TRUE) || packageVersion(pkg) < swat62_versions[[pkg]]) {
      stop('Install ', pkg, ' >= ', swat62_versions[[pkg]], ' from MR-Eini/', pkg, '-swat62 before continuing.')
    }
  }
}
swat62_executable <- function() {
  explicit <- Sys.getenv('SWAT_EXE')
  paths <- c(explicit, file.path(swat_workflow_root(),
    'package-updates/tools/ifort62/swatplus-62-ifo-win_amd64-Rel.exe'),
